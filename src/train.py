...
class Trainer:
    def __init__(self, params):
        ...
        self.x_train = torch.load(data_path + '/x_train.pt').float()
        self.y_train = torch.load(data_path + '/y_train.pt').float()
        self.x_valid = torch.load(data_path + '/x_valid.pt').float()
        self.y_valid = torch.load(data_path + '/y_valid.pt').float()

        underground_channels = params['underground_channels']
        self.underground_data = torch.load(underground_data_path).float()[:, underground_channels, :, :]
        ...
        self.criterion = SSIMLoss(mask=self.mask)
